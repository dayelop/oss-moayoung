...
export class Firebase implements IExchange {
  ...
  constructor(room: string, yourId: number, authenticated: () => void) {
    ...
    firebase
      ...
      .signInAnonymously()
      .then(() => {
        cla.roomDatabase = firebase
          .database()
          .ref('rooms/' + room + '/partners/all');
        cla.ownDatabase = firebase
          .database()
          .ref('rooms/' + room + '/partners/' + yourId);
  ...
  readMessage(data, cla) {
    if (data.val().message) {
      ...
      if (
        dataroom === cla.room &&
        sender !== cla.yourId &&
        (receiver == 0 || receiver == cla.yourId)
      ) {
  ...
  addReadEvent(
    callback: (sender: number, dataroom: string, msg: any) => void
  ): void {
  ...
  getDatabaseRef(receiver: number = 0) {
    ...
    this.partnerDatbases[receiver] = firebase
      .database()
      .ref('rooms/' + this.room + '/partners/' + receiver);
    return this.partnerDatbases[receiver];
  }
}
