...
export class Partner implements IPartner {
  ...
  setScreenSharing(screenSharing: boolean) {
    ...
    this.partnerListElement.partnerListElementVueObject.screenSharing =
      screenSharing;
  }
  ...
  checkConnection() {
    ...
    setInterval(function () {
      if (!cla.connected && !cla.closed && !cla.checking) {
        ...
        if (
          cla.lastPing.getTime() <=
            new Date().getTime() - cla.millisecondsBeforeHide &&
          cla.lastConnectionLost.getTime() <=
            new Date().getTime() - cla.millisecondsBeforeHide
        ) {
  ...
  createOffer(doLoop: boolean = false): void {
    if ((!this.connected && !this.closed) || this.doReload) {
      ...
      this.connection
        ...
        .then(function () {
          cla.exchange.sendMessage(
            { sdp: cla.connection.localDescription },
            cla.id
          );
        });
    }
  ...
  onMessage(message: any, partner: IPartner) {
    ...
    if (message.type !== undefined && message.message !== undefined) {
      if (message.type === partner.textchat.textchatMessageType) {
        ...
        message.type === Userinfo.userinfoMessageType &&
  ...
  addVideoElement() {
    ...
    if (this.videoElement == undefined) {
      $('#video-area').append(
        '<div class="video-item video-item-partner unconnected" id="video-item-' +
          this.id +
          '"><div class="video-wrap"><div class="video-inner-wrap"><video id="video-' +
          this.id +
          '" autoplay playsinline></video></div></div></div>'
      );
      this.videoElement = document.getElementById('video-' + this.id);
      this.videoGridElement = new Video(
        document.getElementById('video-item-' + this.id),
        this
      );
