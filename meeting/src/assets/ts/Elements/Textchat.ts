...
export class Textchat {
      ...
      methods: {
        sendMessage: function () {
          if (this.image) {
            ...
            cla.app.sendMessageToAllPartners({
              type: cla.textchatMessageType,
  ...
  initialDatabase() {
    ...
    this.database.initDatabase(
      this.textchatDatabaseName,
      this.dbVersion,
      this.databaseStructure,
      function (success: boolean, caller: Textchat) {
        if (success) {
  ...
  loadMessagesFromDB() {
    ...
    this.database.read(
      this.databaseStructure[this.textchatDatabaseObjectName],
      query,
      function (success: boolean, data: any, caller: Textchat) {
        if (success) {
          for (var i = 0; i < data.length; i++) {
            caller.addMessageToChat(
              data[i].partnerName,
              data[i].message,
              data[i].date,
              data[i].self,
  ...
  addMessage(
    ...
    self: boolean = false,
    type: string = this.textchatMessageTypeText
  ) {
  ...
  scrollToBottom() {
    $('#textchat .msger-chat').scrollTop(
      $('#textchat .msger-chat').get(0).scrollHeight
    );
  }
